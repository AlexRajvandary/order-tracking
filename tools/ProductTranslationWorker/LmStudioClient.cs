...
public sealed class LmStudioClient(HttpClient http, Microsoft.Extensions.Options.IOptions<LmStudioOptions> options)
{
    ...
    {
        var prompt = TranslationPrompt.Build(products);
        using var response = await http.PostAsJsonAsync("chat/completions", new { model = o.Model, temperature = o.Temperature, messages = new[] { new { role = "system", content = TranslationPrompt.SystemMessage }, new { role = "user", content = prompt } } }, ct);
