    ...
    <div className="min-h-screen bg-background"><SiteHeader /><main className="mx-auto w-full max-w-6xl px-4 py-6 sm:py-8">
      ...
      <div className="grid gap-10 lg:grid-cols-[minmax(0,1.15fr)_minmax(22rem,0.85fr)] lg:gap-14">
        ...
        <div className="flex flex-col">
          ...
          <div className="mt-6 space-y-3 bg-muted/40 p-5 text-sm"><p className="font-medium">Заказ из Японии</p><p className="text-muted-foreground">Товар будет выкуплен у японского магазина после оформления заказа.</p><div className="flex justify-between border-t border-border pt-3"><span>Доставка</span><span>7–14 дней</span></div><div className="flex justify-between"><span>Состояние</span><span>{condition || "—"}</span></div><Button render={<Link href="/item-weight" />} variant="outline" className="mt-2 w-full bg-background"><Scale data-icon="inline-start" />Примерный вес товара</Button></div>
        </div>
      </div>
