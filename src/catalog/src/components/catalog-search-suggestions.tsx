...
type Suggestions = { localProducts: ApiProduct[]; rakutenProducts: ApiExternalProduct[] };
...
export function CatalogSearchSuggestions({ categories, value, onChange, onNavigate }: {
  ...
  const [suggestions, setSuggestions] = useState<Suggestions>({ localProducts: [], rakutenProducts: [] });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      catch (error) { if (!(error instanceof DOMException && error.name === "AbortError")) setSuggestions({ localProducts: [], rakutenProducts: [] }); }
  ...
  return <div ref={root} className="relative mb-3">
    ...
    {open && normalized.length >= 2 ? <div className="absolute top-full left-0 z-50 mt-2 w-[min(760px,calc(100vw-2rem))] overflow-hidden rounded-xl border border-[#E5E7EB] bg-white shadow-xl">
      {!loading && !hasResults ? <p className="p-5 text-sm text-muted-foreground">Ничего не найдено</p> : <div className="max-h-[70vh] overflow-y-auto p-4">
        ...
        {suggestions.localProducts.length ? <section className="mt-4 border-t pt-4"><h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Товары TheGet</h3>
        ...
        {suggestions.rakutenProducts.length ? <section className="mt-4 border-t pt-4"><div className="mb-2 flex items-center justify-between"><h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Товары Rakuten</h3><Link href={`/rakuten?query=${encodeURIComponent(value.trim())}`} onClick={onNavigate} className="text-sm font-medium text-primary hover:underline">Смотреть всё</Link></div>
